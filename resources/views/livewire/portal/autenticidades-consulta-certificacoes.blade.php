<div class="py-5">
        <x-slot:title>
            Consulta de autenticidade de certificação: {{ $alvara->Solicitacao->num_protocolo }}
        </x-slot:title>

    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-1 space-y-6">

        {{-- CARD PRINCIAPL --}}
        <x-card header="Informações da certificação do protocolo: {{ $alvara->Solicitacao->num_protocolo }}" color="primary" bordered>
            <x-card>
                {{-- SHOW SOLICITAÇÃO --}}
                <div class="space-y-6 ">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <p class="text-base font-semibold ">
                                {{ $alvara->Solicitacao->num_protocolo ?? '-' }}
                            </p>
                        </div>
                        ...
                        <div>
                            ...
                            <p class="text-base font-semibold">
                                {{ $alvara->Solicitacao->empresa_cpf_cnpj ?? '-' }}
                            </p>
                        </div>
                    </div>
                    <!-- Linha 3 -->
                    <div class="grid grid-cols-1">
                        <div>
                            ...
                            <p class="text-base font-semibold">
                                {{ $alvara->Solicitacao->empresa_razao_social ?? '-' }}
                            </p>
                        </div>
                    ...
                    <div class="grid grid-cols-1">
                        <div>
                            ...
                            <p class="text-base font-semibold">
                                {{ $alvara->Solicitacao->empresa_nome_fantasia ?? '-' }}
                            </p>
                        </div>
                    ...
                    <div class="grid grid-cols-1">
                        <div>
                            ...
                            <p class="text-base leading-relaxed">
                                {{ $alvara->Solicitacao->endereco_logradouro ?? '-' }}, {{ $alvara->Solicitacao->endereco_numero ?? '-' }} 
                                {{ $alvara->Solicitacao->endereco_complemento ?? '' }}, {{ $alvara->Solicitacao->endereco_bairro ?? '-' }},
                                {{ $alvara->Solicitacao->endereco_municipio ?? '-' }} - {{ $alvara->Solicitacao->endereco_estado ?? '-' }}
                            </p>
                        </div>
                    </div>
